...
<div class="page">
    ...
    <div class="notice-list" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="50">
       <div class="notice" v-for="item of list" :key="item.id">
           <div class="notice-time">{{item.add_time|dateFmt}}</div>
           <div class="notice-body">
               <div class="notice-title">{{item.type === 1 ? '+' : '-'}}{{item.points}}</div>
               <div class="notice-content">{{item.content}}</div>
           </div>
       </div>
    </div>
    <div class="empty-list" v-show="list.length === 0">
        没有查询到结果
    </div>
</div>
</template>
